from app import db, app, Region, Comuna

# Datos de regiones y comunas extraídos de tu JavaScript
REGIONES_COMUNAS = {
    "regiones": [
        {
...
def cargar_regiones_comunas():
    with app.app_context():
        ...
        if Region.query.first():
            print("✅ Regiones y comunas ya existen en la base de datos")
            return
        
        print("Cargando regiones y comunas...")
        
        for region_data in REGIONES_COMUNAS["regiones"]:
            region = Region(nombre=region_data["nombre"])
            db.session.add(region)
            db.session.flush()  # Para obtener el ID de la región
        ...
        comuna_count = Comuna.query.count()
        print(f"📊 Estadísticas: {region_count} regiones, {comuna_count} comunas")
